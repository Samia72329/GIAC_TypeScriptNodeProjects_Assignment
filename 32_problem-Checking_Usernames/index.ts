...
let current_users : string[] = ["Admin", "Guest", "Micheal Jackson", "Charles Leclerc", "Gabriel", "Nikola Tesla"];
let new_users : string[] = ["Jeon Jungkook", "Doremon", "MiCheal JaCKson", "Gojo Saturou", "Light Yagami", "nikola tesla", "guESt"];
new_users.forEach((element) => {
    if(current_users.some((user) => user.toLowerCase() === element.toLowerCase())) {
        console.log(`The username: ${element} is already taken`)
    } else {
